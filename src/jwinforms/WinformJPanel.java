...
public class WinformJPanel extends JPanel {
	...
	public WinformJPanel(WinformPane form) {
		...
		setFocusTraversalPolicy(new SortingFocusTraversalPolicy(
				new Comparator<Component>() {
					@Override
					public int compare(Component o1, Component o2) {
						return tabOrderMap.get(o1).compareTo(
								tabOrderMap.get(o2));
					}
				}) {
			@Override
	...
	protected void paintComponent(Graphics g) {
		...
		if (BackgroundImage != null) {
			g.drawImage(BackgroundImage.asSwingImage(), 0, 0, this.getWidth(),
					this.getHeight(), null);
		}
	}
	...
	void handleDialogResult(final Button button) {
		if (button.DialogResult != null)
			button.setClick(new ChainedEventHandler<Object, EventArgs>(
					button.click) {
				@Override
