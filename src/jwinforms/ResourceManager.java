...
public class ResourceManager {
	...
	private static final ClassLoader classLoader = ResourceManager.class
			.getClassLoader();
	...
	protected ResourceManager(URL resource, String path) {
		try {
			...
			throw new Error(e.getMessage() + ": trying to load url \""
					+ resource + "\"", e);
		}
	}

	public ResourceManager(Class<?> className) {
		this(classLoader.getResource(classToPath(className)
				+ className.getSimpleName() + ".properties"),
	...
	public Object GetObject(String key) {
		...
		if (objectType.equals("ImageListStreamer")) {
			...
			try {
				return new ImageStreamResourceManager(
						classLoader.getResource(path + streamFilename), path)
						.getStream();
				// return new
				// ImageStreamResourceManager(classLoader.getResource(streamFilename)).getStream();
