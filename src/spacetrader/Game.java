...
public class Game extends STSerializableObject implements SpaceTraderGame,
	...
	private EncounterType _encounterType = spacetrader.enums.EncounterType
			.FromInt(0); // Type
	// of
	// current
	// encounter
	...
	public Game(String name, Difficulty difficulty, int pilot, int fighter,
			int trader, int engineer, MainWindow parentWin) {
		_game = this;
		...
		_commander = InitializeCommander(name, new CrewMember(
				CrewMemberId.Commander, pilot, fighter, trader, engineer,
				StarSystemId.NA));
		...
		if (Difficulty().CastToInt() < Difficulty.Normal.CastToInt())
			Commander().getCurrentSystem().SpecialEventType(
	...
	public Game(Hashtable hash, MainWindow parentWin) {
		...
		_universe = (StarSystem[]) ArrayListToArray(
				GetValueFromHash(hash, "_universe", ArrayList.class),
				"StarSystem");
		_wormholes = GetValueFromHash(hash, "_wormholes", _wormholes,
				int[].class);
		_mercenaries = (CrewMember[]) ArrayListToArray(
				GetValueFromHash(hash, "_mercenaries", ArrayList.class),
				"CrewMember");
		_commander = new Commander(GetValueFromHash(hash, "_commander",
				Hashtable.class));
		_dragonfly = new Ship(GetValueFromHash(hash, "_dragonfly",
				_dragonfly.Serialize(), Hashtable.class));
		_scarab = new Ship(GetValueFromHash(hash, "_scarab",
				_scarab.Serialize(), Hashtable.class));
		_scorpion = new Ship(GetValueFromHash(hash, "_scorpion",
				_scorpion.Serialize(), Hashtable.class));
		_spaceMonster = new Ship(GetValueFromHash(hash, "_spaceMonster",
				_spaceMonster.Serialize(), Hashtable.class));
		_opponent = new Ship(GetValueFromHash(hash, "_opponent",
				_opponent.Serialize(), Hashtable.class));
		_chanceOfTradeInOrbit = GetValueFromHash(hash, "_chanceOfTradeInOrbit",
				_chanceOfTradeInOrbit);
		...
		_tribbleMessage = GetValueFromHash(hash, "_tribbleMessage",
				_tribbleMessage);
		_arrivedViaWormhole = GetValueFromHash(hash, "_arrivedViaWormhole",
				_arrivedViaWormhole);
		_paidForNewspaper = GetValueFromHash(hash, "_paidForNewspaper",
				_paidForNewspaper);
		...
		_cheats = new GameCheats(this);
		_cheats.cheatMode = GetValueFromHash(hash, "_cheatEnabled",
				_cheats.cheatMode);
		_autoSave = GetValueFromHash(hash, "_autoSave", _autoSave);
		_easyEncounters = GetValueFromHash(hash, "_easyEncounters",
				_easyEncounters);
		_endStatus = GameEndType.FromInt(GetValueFromHash(hash, "_endStatus",
				_endStatus, Integer.class));
		_encounterType = EncounterType.FromInt(GetValueFromHash(hash,
				"_encounterType", _encounterType, Integer.class));
		_selectedSystemId = StarSystemId.FromInt(GetValueFromHash(hash,
				"_selectedSystemId", _selectedSystemId, Integer.class));
		_warpSystemId = StarSystemId.FromInt(GetValueFromHash(hash,
				"_warpSystemId", _warpSystemId, Integer.class));
		_trackedSystemId = StarSystemId.FromInt(GetValueFromHash(hash,
				"_trackedSystemId", _trackedSystemId, Integer.class));
		_targetWormhole = GetValueFromHash(hash, "_targetWormhole",
				_targetWormhole);
		_priceCargoBuy = GetValueFromHash(hash, "_priceCargoBuy",
				_priceCargoBuy, int[].class);
		_priceCargoSell = GetValueFromHash(hash, "_priceCargoSell",
				_priceCargoSell, int[].class);
		_questStatusArtifact = GetValueFromHash(hash, "_questStatusArtifact",
				_questStatusArtifact);
		_questStatusDragonfly = GetValueFromHash(hash, "_questStatusDragonfly",
				_questStatusDragonfly);
		_questStatusExperiment = GetValueFromHash(hash,
				"_questStatusExperiment", _questStatusExperiment);
		_questStatusGemulon = GetValueFromHash(hash, "_questStatusGemulon",
				_questStatusGemulon);
		_questStatusJapori = GetValueFromHash(hash, "_questStatusJapori",
				_questStatusJapori);
		_questStatusJarek = GetValueFromHash(hash, "_questStatusJarek",
				_questStatusJarek);
		_questStatusMoon = GetValueFromHash(hash, "_questStatusMoon",
				_questStatusMoon);
		_questStatusPrincess = GetValueFromHash(hash, "_questStatusPrincess",
				_questStatusPrincess);
		_questStatusReactor = GetValueFromHash(hash, "_questStatusReactor",
				_questStatusReactor);
		_questStatusScarab = GetValueFromHash(hash, "_questStatusScarab",
				_questStatusScarab);
		_questStatusSculpture = GetValueFromHash(hash, "_questStatusSculpture",
				_questStatusSculpture);
		_questStatusSpaceMonster = GetValueFromHash(hash,
				"_questStatusSpaceMonster", _questStatusSpaceMonster);
		_questStatusWild = GetValueFromHash(hash, "_questStatusWild",
				_questStatusWild);
		_fabricRipProbability = GetValueFromHash(hash, "_fabricRipProbability",
				_fabricRipProbability);
		_justLootedMarie = GetValueFromHash(hash, "_justLootedMarie",
				_justLootedMarie);
		_canSuperWarp = GetValueFromHash(hash, "_canSuperWarp", _canSuperWarp);
		_chanceOfVeryRareEncounter = GetValueFromHash(hash,
				"_chanceOfVeryRareEncounter", _chanceOfVeryRareEncounter);
		_veryRareEncounters = new ArrayList(Arrays.asList(GetValueFromHash(
				hash, "_veryRareEncounters",
				_veryRareEncounters.toArray(new Integer[0]))));
		_options = new GameOptions(GetValueFromHash(hash, "_options",
				_options.Serialize(), Hashtable.class));
	}

	private void Arrested() {
		int term = Math.max(30, -Commander().getPoliceRecordScore());
		int fine = (1 + Commander().Worth()
				* Math.min(80, -Commander().getPoliceRecordScore()) / 50000) * 500;
		if (Commander().getShip().WildOnBoard())
			fine = (int) (fine * 1.05);

		GuiFacade.alert(AlertType.EncounterArrested);

		GuiFacade.alert(AlertType.JailConvicted,
				Functions.Multiples(term, Strings.TimeUnit),
		...
		if (Commander().getShip().HasGadget(GadgetType.HiddenCargoBays)) {
			while (Commander().getShip().HasGadget(GadgetType.HiddenCargoBays))
				Commander().getShip().RemoveEquipment(EquipmentType.Gadget,
		...
		if (Commander().getShip().CrewCount()
				- Commander().getShip().SpecialCrew().length > 1) {
		...
		if (Commander().getCash() >= fine)
			...
			Commander().setCash(
					Math.max(0, Commander().getCash()
							+ Commander().getShip().Worth(true) - fine));
		...
		if (Commander().getDebt() > 0) {
			int paydown = Math
					.min(Commander().getCash(), Commander().getDebt());
	...
	private void Arrival() {
		...
		if (TrackedSystem() == Commander().getCurrentSystem()
				&& Options().getTrackAutoOff())
	...
	private void ArrivalCheckDebt() {
		...
		else if (Commander().getDebt() > 0 && Options().getRemindLoans()
				&& Commander().getDays() % 5 == 0)
			GuiFacade.alert(AlertType.DebtReminder, Functions.Multiples(
					Commander().getDebt(), Strings.MoneyUnit));
	}
	...
	private void ArrivalCheckEasterEgg() {
		...
		if (Commander().getCurrentSystem().Id() == StarSystemId.Og) {
			...
			Commander().getShip().AddEquipment(
	...
	private void ArrivalCheckTribbles() {
		...
		if (ship.getTribbles() > 0) {
			...
			if (ship.ReactorOnBoard()) {
				...
				int dead = Math.min(1 + Functions.GetRandom(3),
						ship.Cargo()[narc]);
				Commander().PriceCargo()[narc] = Commander().PriceCargo()[narc]
						* (ship.Cargo()[narc] - dead) / ship.Cargo()[narc];
				ship.Cargo()[narc] -= dead;
				ship.Cargo()[TradeItemType.Furs.CastToInt()] += dead;
				ship.setTribbles(ship.getTribbles()
						- Math.min(dead * (Functions.GetRandom(5) + 98),
								ship.getTribbles() - 1));
				GuiFacade.alert(AlertType.TribblesMostDied);
			} else {
				if (ship.Cargo()[food] > 0
						&& ship.getTribbles() < Consts.MaxTribbles) {
					int eaten = ship.Cargo()[food]
							- Functions.GetRandom(ship.Cargo()[food]);
					Commander().PriceCargo()[food] -= Commander().PriceCargo()[food]
							* eaten / ship.Cargo()[food];
				...
				if (ship.getTribbles() < Consts.MaxTribbles)
					ship.setTribbles(ship.getTribbles()
							+ (1 + Functions.GetRandom(ship.Cargo()[food] > 0 ? ship
									.getTribbles() : ship.getTribbles() / 2)));
				...
				if ((previousTribbles < 100 && ship.getTribbles() >= 100)
						...
						|| (previousTribbles < Consts.MaxTribbles && ship
								.getTribbles() == Consts.MaxTribbles)) {
					String qty = ship.getTribbles() == Consts.MaxTribbles ? Strings.TribbleDangerousNumber
							: Functions.FormatNumber(ship.getTribbles());
	...
	private void ArrivalPerformRepairs() {
		...
		if (ship.getHull() < ship.HullStrength())
			ship.setHull(ship.getHull()
					+ Math.min(ship.HullStrength() - ship.getHull(),
							Functions.GetRandom(ship.Engineer())));
		...
		if (Options().getAutoFuel() && toAdd > 0) {
			if (Commander().getCash() >= toAdd * ship.getFuelCost()) {
				...
				Commander().setCash(
						Commander().getCash() - (toAdd * ship.getFuelCost()));
			} else
				fuelOk = false;
		}
		...
		if (Options().getAutoRepair() && toAdd > 0) {
			if (Commander().getCash() >= toAdd * ship.getRepairCost()) {
				...
				Commander().setCash(
						Commander().getCash() - (toAdd * ship.getRepairCost()));
			} else
				repairOk = false;
		}
	...
	private void ArrivalUpdatePressuresAndQuantities() {
		for (int i = 0; i < Universe().length; i++) {
			if (Functions.GetRandom(100) < 15)
				Universe()[i]
						.SystemPressure((SystemPressure.FromInt(Universe()[i]
								.SystemPressure() == SystemPressure.None ? Functions.GetRandom(
								SystemPressure.War.CastToInt(),
				...
				if (Universe()[i].CountDown() > CountDownStart())
					...
					for (int j = 0; j < Consts.TradeItems.length; j++) {
						if (WarpSystem().ItemTraded(Consts.TradeItems[j]))
							Universe()[i].TradeItems()[j] = Math.max(
									0,
									Universe()[i].TradeItems()[j]
	...
	private void CalculatePrices(StarSystem system) {
		for (int i = 0; i < Consts.TradeItems.length; i++) {
			...
			if (price > 0) {
				...
				if (Consts.TradeItems[i].PressurePriceHike() == system
						.SystemPressure())
					price = price * 3 / 2;

				// Randomize price a bit
				int variance = Math.min(Consts.TradeItems[i].PriceVariance(),
						price - 1);
	...
	public boolean setEncounterContinueAttacking(
			boolean encounterContinueAttacking) {
	...
	private void CargoBuy(int tradeItem, boolean max, CargoBuyOp op) {
		...
		switch (op) {
		case BuySystem:
			freeBays = Math.max(0, Commander().getShip().FreeCargoBays()
					- Options().getLeaveEmpty());
		...
		case BuyTrader:
			...
			double adj = Functions.GetRandom(100) < chance ? 1.1 : (item
					.Illegal() ? 0.8 : 0.9);
			unitPrice = Math.min(
					item.MaxTradePrice(),
					Math.max(
							item.MinTradePrice(),
							(int) Math.round(PriceCargoBuy()[tradeItem] * adj
									/ item.RoundOff())
									* item.RoundOff()));
			break;
		case Plunder:
		...
		if (op == CargoBuyOp.BuySystem
				&& Commander().getDebt() > Consts.DebtTooLarge)
			GuiFacade.alert(AlertType.DebtTooLargeTrade);
		else if (op == CargoBuyOp.BuySystem
				&& (items[tradeItem] <= 0 || unitPrice <= 0))
			...
			if (op == CargoBuyOp.BuySystem)
				maxAmount = Math.min(maxAmount, Commander().CashToSpend()
						/ unitPrice);
	...
	private void CargoSell(int tradeItem, boolean all, CargoSellOp op) {
		...
		case SellTrader:
			TradeItem item = Consts.TradeItems[tradeItem];
			int chance = item.Illegal() ? 45 : 10;
			double adj = Functions.GetRandom(100) < chance ? (item.Illegal() ? 0.8
					: 0.9)
					: 1.1;
			unitPrice = Math.min(
					item.MaxTradePrice(),
					Math.max(
							item.MinTradePrice(),
							(int) Math.round(PriceCargoSell()[tradeItem] * adj
									/ item.RoundOff())
									* item.RoundOff()));
			break;
		default:
		...
		if (qtyInHand == 0)
			GuiFacade.alert(AlertType.CargoNoneToSell,
			...
			if (op != CargoSellOp.Jettison
				...
				int unitCost = 0;
				int maxAmount = (op == CargoSellOp.SellTrader) ? Math.min(
						qtyInHand, getOpponent().FreeCargoBays()) : qtyInHand;
				...
				if (all)
					...
					qty = GuiFacade.queryAmountRelease(tradeItem, op,
							maxAmount, price);
				}
				...
				if (qty > 0) {
					...
					Commander().getShip().Cargo()[tradeItem] -= qty;
					Commander().PriceCargo()[tradeItem] = (Commander()
							.PriceCargo()[tradeItem] * (qtyInHand - qty))
							/ qtyInHand;
					Commander().setCash(Commander().getCash() + totalPrice);
					...
					if (op == CargoSellOp.Jettison) {
						if (Functions.GetRandom(10) < Difficulty().CastToInt() + 1) {
							if (Commander().getPoliceRecordScore() > Consts.PoliceRecordScoreDubious)
								...
								Commander().setPoliceRecordScore(
										Commander().getPoliceRecordScore() - 1);
	...
	private void CreateShips() {
		Dragonfly().Crew()[0] = Mercenaries()[CrewMemberId.Dragonfly
				.CastToInt()];
		Dragonfly().AddEquipment(
				Consts.Weapons[WeaponType.MilitaryLaser.CastToInt()]);
		Dragonfly().AddEquipment(
		...
		Dragonfly().AddEquipment(
				Consts.Shields[ShieldType.Lightning.CastToInt()]);
		Dragonfly().AddEquipment(
				Consts.Gadgets[GadgetType.AutoRepairSystem.CastToInt()]);
		Dragonfly().AddEquipment(
				Consts.Gadgets[GadgetType.TargetingSystem.CastToInt()]);
		...
		Scorpion().AddEquipment(
				Consts.Weapons[WeaponType.MilitaryLaser.CastToInt()]);
		Scorpion().AddEquipment(
				Consts.Weapons[WeaponType.MilitaryLaser.CastToInt()]);
		Scorpion().AddEquipment(
				Consts.Shields[ShieldType.Reflective.CastToInt()]);
		Scorpion().AddEquipment(
				Consts.Shields[ShieldType.Reflective.CastToInt()]);
		Scorpion().AddEquipment(
				Consts.Gadgets[GadgetType.AutoRepairSystem.CastToInt()]);
		...
		SpaceMonster().Crew()[0] = Mercenaries()[CrewMemberId.SpaceMonster
				.CastToInt()];
		SpaceMonster().AddEquipment(
				Consts.Weapons[WeaponType.MilitaryLaser.CastToInt()]);
		SpaceMonster().AddEquipment(
				Consts.Weapons[WeaponType.MilitaryLaser.CastToInt()]);
		SpaceMonster().AddEquipment(
	...
	private boolean DetermineNonRandomEncounter() {
		...
		if (getClicks() == 1
				&& WarpSystem().Id() == StarSystemId.Acamar
		...
		else if (getArrivedViaWormhole()
				&& getClicks() == 20
				&& WarpSystem().SpecialEventType() != SpecialEventType.NA
		...
		// Encounter with stolen Dragonfly
				...
				&& WarpSystem().Id() == StarSystemId.Zalkon
		...
		// Encounter with kidnappers in the Scorpion
				...
				&& WarpSystem().Id() == StarSystemId.Qonos
	...
	private boolean DeterminePirateEncounter(boolean mantis) {
		...
		if (mantis) {
			...
			// have a very scary
			// reputation.
			else if (getOpponent().Type().CastToInt() > Commander().getShip()
					.Type().CastToInt()
					|| getOpponent().Type().CastToInt() >= ShipType.Grasshopper
							.CastToInt()
					|| Functions.GetRandom(Consts.ReputationScoreElite) > (Commander()
							.getReputationScore() * 4)
			...
			if (Commander().getPoliceRecordScore() < Consts.PoliceRecordScoreDubious) {
				...
				if (getOpponent().WeaponStrength() > 0
						&& (Commander().getReputationScore() < Consts.ReputationScoreAverage || Functions
								.GetRandom(Consts.ReputationScoreElite) > (Commander()
								.getReputationScore() / (1 + getOpponent()
								.Type().CastToInt())))
						|| getOpponent().Type().CastToInt() > Commander()
								.getShip().Type().CastToInt()) {
					...
					&& (Commander().getPoliceRecordScore() < Consts.PoliceRecordScoreClean
							...
							.getPoliceRecordScore() >= Consts.PoliceRecordScoreLawful && Functions
							.GetRandom(40) == 0))) {
	...
	private boolean DetermineRandomEncounter() {
		...
		if (WarpSystem().Id() == StarSystemId.Gemulon
				&& getQuestStatusGemulon() == SpecialEvent.StatusGemulonTooLate) {
			if (Functions.GetRandom(10) > 4)
				mantis = true;
		} else {
			// Check if it is time for an encounter
			int encounter = Functions.GetRandom(44 - (2 * Difficulty()
					.CastToInt()));
			int policeModifier = Math.max(
					1,
					3 - PoliceRecord
							.GetPoliceRecordFromScore(
									Commander().getPoliceRecordScore()).Type()
							.CastToInt());
			...
			if (encounter < WarpSystem().PoliticalSystem().ActivityPirates()
					.CastToInt())
				// When you are already raided, other pirates have little to
				// gain
				pirate = !getRaided();
			else if (encounter < WarpSystem().PoliticalSystem()
					.ActivityPirates().CastToInt()
					+ WarpSystem().PoliticalSystem().ActivityPolice()
							.CastToInt() * policeModifier)
				...
				police = true;
					...
					+ WarpSystem().PoliticalSystem().ActivityPolice()
							.CastToInt()
					* policeModifier
					+ WarpSystem().PoliticalSystem().ActivityTraders()
							.CastToInt())
				trader = true;
			else if (Commander().getShip().WildOnBoard()
					&& WarpSystem().Id() == StarSystemId.Kravat)
				// if you're coming in to Kravat & you have Wild onboard,
				// there'll be swarms o' cops.
				police = Functions.GetRandom(100) < 100 / Math.max(2,
						Math.min(4, 5 - Difficulty().CastToInt()));
			else if (Commander().getShip().ArtifactOnBoard()
					&& Functions.GetRandom(20) <= 3)
				mantis = true;
		}
		...
		else if (Commander().getDays() > 10
				&& Functions.GetRandom(1000) < getChanceOfVeryRareEncounter()
	...
	private boolean DetermineVeryRareEncounter() {
		...
		switch (VeryRareEncounters().get(
				Functions.GetRandom(VeryRareEncounters().size()))) {
		case MarieCeleste:
			// Marie Celeste cannot be at Acamar, Qonos, or Zalkon as it may
			// cause problems with the
			// Space Monster, Scorpion, or Dragonfly
			if (getClicks() > 1
					&& Commander().getCurrentSystemId() != StarSystemId.Acamar
			...
			break;
		case CaptainAhab:
			if (Commander().getShip().HasShield(ShieldType.Reflective)
					&& Commander().Pilot() < 10
					&& Commander().getPoliceRecordScore() > Consts.PoliceRecordScoreCriminal) {
			...
			break;
		case CaptainConrad:
			if (Commander().getShip().HasWeapon(WeaponType.MilitaryLaser, true)
					&& Commander().Engineer() < 10
					&& Commander().getPoliceRecordScore() > Consts.PoliceRecordScoreCriminal) {
	...
	private void EncounterDefeatDragonfly() {
		...
		Commander().setPoliceRecordScore(
				Commander().getPoliceRecordScore() + Consts.ScoreKillPirate);
	...
	public void EncounterDrink() {
		if (GuiFacade.alert(AlertType.EncounterDrinkContents) == DialogResult.Yes) {
			if (getEncounterType() == spacetrader.enums.EncounterType.BottleGood) {
				...
				if (Difficulty().CastToInt() <= spacetrader.enums.Difficulty.Normal
						.CastToInt())
					Commander().IncreaseRandomSkill();
	...
	public EncounterResult EncounterExecuteAction() {
		...
		case TraderAttack:
			setEncounterCmdrHit(EncounterExecuteAttack(getOpponent(),
					Commander().getShip(), getEncounterCmdrFleeing()));
			setEncounterOppHit(!getEncounterCmdrFleeing()
					&& EncounterExecuteAttack(Commander().getShip(),
							getOpponent(), false));
			break;
		case PirateFlee:
		...
		if (Commander().getShip().getHull() <= 0) {
			if (Commander().getShip().getEscapePod())
				...
				GuiFacade
						.alert((getOpponent().getHull() <= 0 ? AlertType.EncounterBothDestroyed
								: AlertType.EncounterYouLose));

				result = EncounterResult.Killed;
			}
		} else if (getOpponentDisabled()) {
			if (getOpponent().Type() == ShipType.Dragonfly
					|| getOpponent().Type() == ShipType.Scarab
				...
				GuiFacade.alert(AlertType.EncounterDisabledOpponent,
						EncounterShipText(), str2);

				Commander().setReputationScore(
						Commander().getReputationScore()
								+ (getOpponent().Type().CastToInt() / 2 + 1));
			...
			if (getEncounterCmdrFleeing()
					&& (Difficulty() == spacetrader.enums.Difficulty.Beginner || (Functions
							.GetRandom(7) + Commander().getShip().Pilot() / 3) * 2 >= Functions
							.GetRandom(getOpponent().Pilot())
							* (2 + Difficulty().CastToInt()))) {
				GuiFacade
						.alert((getEncounterCmdrHit() ? AlertType.EncounterEscapedHit
								: AlertType.EncounterEscaped));
				escaped = true;
			} else if (getEncounterOppFleeing()
					&& Functions.GetRandom(Commander().getShip().Pilot()) * 4 <= Functions
							.GetRandom(7 + getOpponent().Pilot() / 3) * 2) {
			...
			if (escaped)
				...
				if (Options().getContinuousAttack()
						&& (getEncounterCmdrFleeing()
								|| !getEncounterOppFleeing() || Options()
	...
	private boolean EncounterExecuteAttack(Ship attacker, Ship defender,
			boolean fleeing) {
		boolean hit = false;
		...
		// they WILL be hit.
		if (!(Difficulty() == spacetrader.enums.Difficulty.Beginner
				&& defender.CommandersShip() && fleeing)
				&& (attacker.CommandersShip() && getOpponentDisabled()
						&& attacker.HasGadget(GadgetType.TargetingSystem) || Functions
						.GetRandom(attacker.Fighter()
								+ defender.getSize().CastToInt()) >= (fleeing ? 2
						: 1)
			...
			if (attacker.CommandersShip() && getOpponentDisabled())
				...
				int attackerLasers = attacker.WeaponStrength(
						WeaponType.PulseLaser, WeaponType.MorgansLaser);
				int attackerDisruptors = attacker.WeaponStrength(
						WeaponType.PhotonDisruptor,
						WeaponType.QuantumDistruptor);

				if (defender.Type() == ShipType.Scarab) {
					attackerLasers -= attacker.WeaponStrength(
							WeaponType.BeamLaser, WeaponType.MilitaryLaser);
					attackerDisruptors -= attacker.WeaponStrength(
							WeaponType.PhotonDisruptor,
							WeaponType.PhotonDisruptor);
				}
				...
				if (defender.Disableable() && defender.ShieldCharge() == 0
						&& !getOpponentDisabled()
						&& Options().getDisableOpponents()
						&& attackerDisruptors > 0) {
					disrupt = Functions.GetRandom(attackerDisruptors
							* (100 + 2 * attacker.Fighter()) / 100);
				} else {
					int damage = attackerWeapons == 0 ? 0 : Functions
							.GetRandom(attackerWeapons
									* (100 + 2 * attacker.Fighter()) / 100);
					...
					if (damage > 0) {
						...
						if (defender.ReactorOnBoard())
							damage *= (int) (1 + (Difficulty().CastToInt() + 1)
									* (Difficulty().CastToInt() < spacetrader.enums.Difficulty.Normal
											.CastToInt() ? 0.25 : 0.33));

						// First, shields are depleted
						for (int i = 0; i < defender.Shields().length
								&& defender.Shields()[i] != null && damage > 0; i++) {
							int applied = Math.min(
									defender.Shields()[i].getCharge(), damage);
							defender.Shields()[i]
									.setCharge(defender.Shields()[i]
											.getCharge() - applied);
							damage -= applied;
						}
						...
						if (damage > 0) {
							damage = Math.max(
									1,
									damage
											- Functions.GetRandom(defender
													.Engineer()));

							disrupt = damage * attackerDisruptors
									/ attackerWeapons;
							...
							damage = Math
									.min(damage,
											defender.HullStrength()
													/ (defender
															.CommandersShip() ? Math
															.max(1,
																	spacetrader.enums.Difficulty.Impossible
																			.CastToInt()
																			- Difficulty()
																					.CastToInt())
															: 2));
							...
							defender.setHull(Math.max(0, defender.getHull()
									- damage));
						}
					}
				}
				...
				if (defender.getHull() > 0
						...
						&& Functions.GetRandom(100) < disrupt
								* Consts.DisruptorSystemsMultiplier * 100
								/ defender.getHull())
					setOpponentDisabled(true);

				// Make sure the Scorpion doesn't get destroyed.
				if (defender.Type() == ShipType.Scorpion
						&& defender.getHull() == 0) {
					defender.setHull(1);
					setOpponentDisabled(true);
				}
	...
	public void EncounterMeet() {
		...
		if (GuiFacade.alert(initialAlert) == DialogResult.Yes) {
			...
			Commander().Skills()[skill] = Math
					.min(Consts.MaxSkill,
							Commander().Skills()[skill]
									+ (Difficulty().CastToInt() <= spacetrader.enums.Difficulty.Normal
											.CastToInt() ? 2 : 1));
	...
	public void EncounterPlunder() {
		...
		if (getEncounterType().CastToInt() >= spacetrader.enums.EncounterType.TraderAttack
				.CastToInt()) {
			Commander().setPoliceRecordScore(
					Commander().getPoliceRecordScore()
							+ Consts.ScorePlunderTrader);

			if (getOpponentDisabled())
				Commander().setKillsTrader(Commander().getKillsTrader() + 1);
		} else if (getOpponentDisabled()) {
			if (Commander().getPoliceRecordScore() >= Consts.PoliceRecordScoreDubious) {
				GuiFacade.alert(AlertType.EncounterPiratesBounty,
						Strings.EncounterPiratesDisabled,
						Strings.EncounterPiratesLocation, Functions.Multiples(
								getOpponent().Bounty(), Strings.MoneyUnit));

				Commander().setCash(
						Commander().getCash() + getOpponent().Bounty());
			}

			Commander().setKillsPirate(Commander().getKillsPirate() + 1);
			Commander()
					.setPoliceRecordScore(
							Commander().getPoliceRecordScore()
									+ Consts.ScoreKillPirate);
		} else
			Commander().setPoliceRecordScore(
					Commander().getPoliceRecordScore()
	...
	private void EncounterScoop() {
		...
		if ((Difficulty().CastToInt() < spacetrader.enums.Difficulty.Normal
				.CastToInt() || Functions.GetRandom(Difficulty().CastToInt()) == 0)
			...
			if (GuiFacade.alert(AlertType.EncounterScoop,
					Consts.TradeItems[tradeItem].Name()) == DialogResult.Yes) {
	...
	public void EncounterTrade() {
		...
		int item = (buy ? Commander().getShip() : getOpponent())
				.GetRandomTradeableItem();
		...
		if (Commander().getCash() != cash)
			GuiFacade.alert(AlertType.EncounterTradeCompleted, alertStr,
		...
		if (getOpponent().getHull() < prevOppHull || getOpponentDisabled()) {
			...
			case TraderSurrender:
				...
				// get damaged very little or not at all, the trader will flee.
						...
						&& (Commander().getShip().getHull() < (prevCmdrHull * 2) / 3
								&& chance < 20
								|| Commander().getShip().getHull() < (prevCmdrHull * 9) / 10
								&& chance < 60 || Commander().getShip()
								.getHull() >= (prevCmdrHull * 9) / 10))
					setEncounterType(spacetrader.enums.EncounterType.TraderFlee);
				break;
			default:
				break;
	...
	public boolean EncounterVerifyAttack() {
		...
		if (Commander().getShip().WeaponStrength() == 0) {
			...
			attack = false;
				...
				&& Commander().getShip().WeaponStrength(WeaponType.PulseLaser,
						WeaponType.MorgansLaser) == 0) {
			GuiFacade.alert(AlertType.EncounterAttackNoLasers);
			attack = false;
		} else if (getOpponent().Type() == ShipType.Scorpion
				&& Commander().getShip().WeaponStrength(
						WeaponType.PhotonDisruptor,
						WeaponType.QuantumDistruptor) == 0) {
			...
			case SpaceMonsterIgnore:
				setEncounterType(spacetrader.enums.EncounterType
						.FromInt(getEncounterType().CastToInt() - 1));

				break;
			case PoliceInspect:
				if (!Commander().getShip().DetectableIllegalCargoOrPassengers()
						&& GuiFacade
								.alert(AlertType.EncounterPoliceNothingIllegal) != DialogResult.Yes)
					attack = false;
			...
			case TraderAttack:
			case TraderSurrender:
				if (Functions.GetRandom(Consts.ReputationScoreElite) <= Commander()
						.getReputationScore()
						* 10
			...
			case CaptainHuie:
				if (GuiFacade.alert(AlertType.EncounterAttackCaptain) == DialogResult.Yes) {
					if (Commander().getPoliceRecordScore() > Consts.PoliceRecordScoreVillain)
						Commander().setPoliceRecordScore(
								Consts.PoliceRecordScoreVillain);
	...
	public boolean EncounterVerifyBoard() {
		...
		if (GuiFacade.alert(AlertType.EncounterMarieCeleste) == DialogResult.Yes) {
			...
			int narcs = Commander().getShip().Cargo()[TradeItemType.Narcotics
					.CastToInt()];

			GuiFacade.performPlundering();

			if (Commander().getShip().Cargo()[TradeItemType.Narcotics
					.CastToInt()] > narcs)
	...
	public boolean EncounterVerifyBribe() {
		...
		if (getEncounterType() == spacetrader.enums.EncounterType.MarieCelestePolice)
			...
			int diffMod = 10 + 5 * (spacetrader.enums.Difficulty.Impossible
					.CastToInt() - Difficulty().CastToInt());
			int passMod = Commander().getShip().IllegalSpecialCargo() ? (Difficulty()
					.CastToInt() <= spacetrader.enums.Difficulty.Normal
					.CastToInt() ? 2 : 3) : 1;
			...
			int bribe = Math.max(
					...
					Math.min(
							10000,
							(int) Math.ceil((double) Commander().Worth()
									/ WarpSystem().PoliticalSystem()
											.BribeLevel() / diffMod / 100)
									* 100 * passMod));

			if (GuiFacade.alert(AlertType.EncounterPoliceBribe,
					Functions.Multiples(bribe, Strings.MoneyUnit)) == DialogResult.Yes) {
	...
	public boolean EncounterVerifyFlee() {
		...
		if (getEncounterType() != spacetrader.enums.EncounterType.PoliceInspect
			...
			if (getEncounterType() == spacetrader.enums.EncounterType.MarieCelestePolice
				...
				setEncounterType(spacetrader.enums.EncounterType.PoliceAttack);
				Commander().setPoliceRecordScore(
						Math.min(Commander().getPoliceRecordScore() + scoreMod,
								scoreMin));
			}
		}
	...
	public boolean EncounterVerifySubmit() {
		...
		if (Commander().getShip().DetectableIllegalCargoOrPassengers()) {
			String str1 = Commander().getShip().IllegalSpecialCargoDescription(
					"", true, true);
			...
			if (GuiFacade.alert(AlertType.EncounterPoliceSubmit, str1, str2) == DialogResult.Yes) {
				...
				if (Commander().getShip().DetectableIllegalCargo()) {
					...
					Commander().setDebt(
							Commander().getDebt() + (fine - cashPayment));
					Commander().setCash(Commander().getCash() - cashPayment);

					GuiFacade.alert(AlertType.EncounterPoliceFine,
							Functions.Multiples(fine, Strings.MoneyUnit));

					Commander().setPoliceRecordScore(
							Commander().getPoliceRecordScore()
									+ Consts.ScoreTrafficking);
				}
			}
		} else {
			submit = true;
		...
		if (getOpponent().Type() == ShipType.Mantis) {
			...
			if (Commander().getPoliceRecordScore() <= Consts.PoliceRecordScorePsychopath)
				GuiFacade.alert(AlertType.EncounterSurrenderRefused);
					...
					.alert(AlertType.EncounterPoliceSurrender,
							(new String[] {
									Commander()
											.getShip()
											.IllegalSpecialCargoDescription(
													Strings.EncounterPoliceSurrenderCargo,
													true, false),
									Commander().getShip()
											.IllegalSpecialCargoActions() })) == DialogResult.Yes)
			...
			if (Commander().getShip().HasGadget(GadgetType.HiddenCargoBays)) {
				...
				GuiFacade.alert(AlertType.PreciousHidden, Functions.StringVars(
						Strings.ListStrings[precious.size()],
			...
			if (cargoToSteal.size() == 0) {
				...
				Commander().setDebt(
						Commander().getDebt() + (blackmail - cashPayment));
				Commander().setCash(Commander().getCash() - cashPayment);
				GuiFacade.alert(AlertType.EncounterPiratesFindNoCargo,
				...
				while (getOpponent().FreeCargoBays() > 0
						&& cargoToSteal.size() > 0) {
	...
	private void EncounterWon() {
		if (getEncounterType().CastToInt() >= spacetrader.enums.EncounterType.PirateAttack
				.CastToInt()
				&& getEncounterType().CastToInt() <= spacetrader.enums.EncounterType.PirateDisabled
						.CastToInt()
				&& getOpponent().Type() != ShipType.Mantis
				&& Commander().getPoliceRecordScore() >= Consts.PoliceRecordScoreDubious)
			GuiFacade.alert(AlertType.EncounterPiratesBounty,
					Strings.EncounterPiratesDestroyed, "", Functions.Multiples(
							getOpponent().Bounty(), Strings.MoneyUnit));
		else
		...
		case FamousCaptainAttack:
			...
			if (Commander().getReputationScore() < Consts.ReputationScoreDangerous)
				...
				Commander().setReputationScore(
						Commander().getReputationScore()
								+ Consts.ScoreKillCaptain);

			// bump news flag from attacked to ship destroyed
			NewsReplaceEvent(NewsLatestEvent(),
					NewsEvent.FromInt(NewsLatestEvent().CastToInt() + 1));
			break;
		case DragonflyAttack:
			EncounterDefeatDragonfly();
			break;
	...
	public void EscapeWithPod() {
		...
		if (getQuestStatusJapori() == SpecialEvent.StatusJaporiInTransit) {
			...
			for (system = 0; system < Universe().length
				...
				;
			GuiFacade.alert(AlertType.AntidoteDestroyed,
		...
		if (Commander().getInsurance()) {
			...
			Commander().setCash(
					Commander().getCash()
							+ Commander().getShip().BaseWorth(true));
		}

		if (Commander().getCash() > Consts.FleaConversionCost)
			Commander().setCash(
					Commander().getCash() - Consts.FleaConversionCost);
		else {
			Commander().setDebt(
					Commander().getDebt()
							+ (Consts.FleaConversionCost - Commander()
									.getCash()));
			Commander().setCash(0);
		}
	...
	private boolean FindDistantSystem(StarSystemId baseSystem,
			SpecialEventType specEvent) {
		int bestDistance = 999;
		int system = -1;
		for (int i = 0; i < Universe().length; i++) {
			int distance = Functions.Distance(
					Universe()[baseSystem.CastToInt()], Universe()[i]);
	...
	private void GenerateCrewMemberList() {
		...
		Mercenaries()[CrewMemberId.Zeethibal.CastToInt()] = new CrewMember(
				CrewMemberId.Zeethibal, 5, 5, 5, 5, StarSystemId.NA);
		Mercenaries()[CrewMemberId.Opponent.CastToInt()] = new CrewMember(
				CrewMemberId.Opponent, 5, 5, 5, 5, StarSystemId.NA);
		Mercenaries()[CrewMemberId.Wild.CastToInt()] = new CrewMember(
				CrewMemberId.Wild, 7, 10, 2, 5, StarSystemId.NA);
		Mercenaries()[CrewMemberId.Jarek.CastToInt()] = new CrewMember(
				CrewMemberId.Jarek, 3, 2, 10, 4, StarSystemId.NA);
		Mercenaries()[CrewMemberId.Princess.CastToInt()] = new CrewMember(
				CrewMemberId.Princess, 4, 3, 8, 9, StarSystemId.NA);
		Mercenaries()[CrewMemberId.FamousCaptain.CastToInt()] = new CrewMember(
				CrewMemberId.FamousCaptain, 10, 10, 10, 10, StarSystemId.NA);
		Mercenaries()[CrewMemberId.Dragonfly.CastToInt()] = new CrewMember(
				CrewMemberId.Dragonfly, 4 + d, 6 + d, 1, 6 + d, StarSystemId.NA);
		Mercenaries()[CrewMemberId.Scarab.CastToInt()] = new CrewMember(
				CrewMemberId.Scarab, 5 + d, 6 + d, 1, 6 + d, StarSystemId.NA);
		Mercenaries()[CrewMemberId.Scorpion.CastToInt()] = new CrewMember(
				CrewMemberId.Scorpion, 8 + d, 8 + d, 1, 6 + d, StarSystemId.NA);
		Mercenaries()[CrewMemberId.SpaceMonster.CastToInt()] = new CrewMember(
				CrewMemberId.SpaceMonster, 8 + d, 8 + d, 1, 1 + d,
				StarSystemId.NA);
		...
		for (int i = 1; i < Mercenaries().length; i++) {
			...
			if (Mercenaries()[i] == null) {
				...
				do {
					id = StarSystemId.FromInt(Functions
				...
				Mercenaries()[i] = new CrewMember(CrewMemberId.FromInt(i),
						Functions.RandomSkill(), Functions.RandomSkill(),
						Functions.RandomSkill(), Functions.RandomSkill(), id);
			}
		}
	}
	...
	private void GenerateUniverse() {
		...
		for (i = 0; i < Universe().length; i++) {
			...
			Size size = Size
					.FromInt(Functions.GetRandom(Size.Huge.CastToInt() + 1));
			PoliticalSystem polSys = Consts.PoliticalSystems[Functions
					.GetRandom(Consts.PoliticalSystems.length)];
			TechLevel tech = TechLevel.FromInt(Functions.GetRandom(polSys
					.MinimumTechLevel().CastToInt(), polSys.MaximumTechLevel()
					.CastToInt() + 1));

			// Galvon must be a Monarchy.
			if (id == StarSystemId.Galvon) {
				size = Size.Large;
				polSys = Consts.PoliticalSystems[PoliticalSystemType.Monarchy
						.CastToInt()];
				tech = TechLevel.HiTech;
			}

			if (Functions.GetRandom(100) < 15)
				pressure = SystemPressure.FromInt(Functions.GetRandom(
						SystemPressure.War.CastToInt(),
						SystemPressure.Employment.CastToInt() + 1));
			if (Functions.GetRandom(5) >= 3)
				specRes = SpecialResource.FromInt(Functions.GetRandom(
						SpecialResource.MineralRich.CastToInt(),
			...
			if (i < Wormholes().length) {
				...
				x = ((Consts.GalaxyWidth * (1 + 2 * (i % 3))) / 6)
						- Functions.GetRandom(-Consts.CloseDistance + 1,
								Consts.CloseDistance);
			...
			Universe()[i] = new StarSystem(id, x, y, size, tech, polSys.Type(),
					pressure, specRes);
		}
	...
	public void HandleSpecialEvent() {
		...
		case CargoForSale:
			...
			Commander().PriceCargo()[tradeItem] += Commander()
					.getCurrentSystem().SpecialEvent().Price();
			break;
		case Dragonfly:
		...
		case Jarek:
			if (Commander().getShip().FreeCrewQuarters() == 0) {
				...
				GuiFacade
						.alert(AlertType.SpecialPassengerOnBoard, jarek.Name());
		...
		case PrincessQonos:
			if (Commander().getShip().FreeCrewQuarters() == 0) {
				...
				CrewMember princess = Mercenaries()[CrewMemberId.Princess
						.CastToInt()];
				GuiFacade.alert(AlertType.SpecialPassengerOnBoard,
						princess.Name());
		...
		case PrincessQuantum:
			if (Commander().getShip().FreeSlotsWeapon() == 0) {
				...
				Commander().getShip()
						.AddEquipment(
								Consts.Weapons[WeaponType.QuantumDistruptor
										.CastToInt()]);
		...
		case Reactor:
			if (Commander().getShip().FreeCargoBays() < 15) {
				...
				if (Commander().getShip().WildOnBoard()) {
					if (GuiFacade.alert(AlertType.WildWontStayAboardReactor,
							curSys.Name()) == DialogResult.OK) {
						GuiFacade
								.alert(AlertType.WildLeavesShip, curSys.Name());
						setQuestStatusWild(SpecialEvent.StatusWildNotStarted);
					} else
						remove = false;
				}
		...
		case ScarabUpgradeHull:
			...
			Commander().getShip().setHull(
					Commander().getShip().getHull() + Consts.HullUpgrade);
		...
		case TribbleBuyer:
			...
			Commander().setCash(
					Commander().getCash()
							+ (Commander().getShip().getTribbles() / 2));
		...
		case Wild:
			if (Commander().getShip().FreeCrewQuarters() == 0) {
				GuiFacade.alert(AlertType.SpecialNoQuarters);
				remove = false;
			} else if (!Commander().getShip().HasWeapon(WeaponType.BeamLaser,
					false)) {
		...
		case WildGetsOut:
			...
			CrewMember zeethibal = Mercenaries()[CrewMemberId.Zeethibal
					.CastToInt()];
			zeethibal.setCurrentSystem(Universe()[StarSystemId.Kravat
					.CastToInt()]);
			int lowest1 = Commander().NthLowestSkill(1);
			int lowest2 = Commander().NthLowestSkill(2);
			for (int i = 0; i < zeethibal.Skills().length; i++)
				zeethibal.Skills()[i] = (i == lowest1 ? 10 : (i == lowest2 ? 8
						: 5));
		...
		if (curSys.SpecialEvent().Price() != 0)
			Commander().setCash(
					Commander().getCash() - curSys.SpecialEvent().Price());
	...
	public void IncDays(int num) {
		...
		if (Commander().getPoliceRecordScore() > Consts.PoliceRecordScoreClean)
			Commander().setPoliceRecordScore(
					Math.max(Consts.PoliceRecordScoreClean, Commander()
							.getPoliceRecordScore() - num / 3));
		...
		if (SpaceMonster().getHull() < SpaceMonster().HullStrength())
			SpaceMonster().setHull(
					Math.min(SpaceMonster().HullStrength(),
							(int) (SpaceMonster().getHull() * Math.pow(1.05,
									num))));

		if (getQuestStatusGemulon() > SpecialEvent.StatusGemulonNotStarted
				&& getQuestStatusGemulon() < SpecialEvent.StatusGemulonTooLate) {
			setQuestStatusGemulon(Math.min(getQuestStatusGemulon() + num,
					SpecialEvent.StatusGemulonTooLate));
			if (getQuestStatusGemulon() == SpecialEvent.StatusGemulonTooLate) {
				StarSystem gemulon = Universe()[StarSystemId.Gemulon
						.CastToInt()];
		...
		if (Commander().getShip().ReactorOnBoard())
			setQuestStatusReactor(Math.min(getQuestStatusReactor() + num,
					SpecialEvent.StatusReactorDate));
		...
		if (getQuestStatusExperiment() > SpecialEvent.StatusExperimentNotStarted
			...
			setQuestStatusExperiment(Math.min(getQuestStatusExperiment() + num,
					SpecialEvent.StatusExperimentPerformed));
			if (getQuestStatusExperiment() == SpecialEvent.StatusExperimentPerformed) {
				...
				Universe()[StarSystemId.Daled.CastToInt()]
						.SpecialEventType(SpecialEventType.ExperimentFailed);
	...
	private Commander InitializeCommander(String name,
			CrewMember commanderCrewMember) {
		...
		while (commander.getCurrentSystem() == null) {
			...
			if (system.SpecialEventType() == SpecialEventType.NA
					&& system.TechLevel().CastToInt() > TechLevel.PreAgricultural
							.CastToInt()
					&& system.TechLevel().CastToInt() < TechLevel.HiTech
							.CastToInt()) {
				// Make sure at least three other systems can be reached
				int close = 0;
				for (int i = 0; i < Universe().length && close < 3; i++) {
					if (i != system.Id().CastToInt()
							&& Functions.Distance(Universe()[i], system) <= commander
									.getShip().FuelTanks())
						close++;
				}
	...
	private void NormalDeparture(int fuel) {
		Commander()
				.setCash(
						Commander().getCash()
								- (MercenaryCosts() + InsuranceCosts() + WormholeCosts()));
	...
	private boolean PlaceShipyards() {
		...
		if (systemIdList.size() < Consts.Shipyards.length)
			...
			for (int shipyard = 0; shipyard < Consts.Shipyards.length; shipyard++)
				Universe()[(Integer) systemIdList.get(Functions
						.GetRandom(systemIdList.size()))].ShipyardId(ShipyardId
						.FromInt(shipyard));
		}
	...
	private boolean PlaceSpecialEvents() {
		...
		if (goodUniverse) {
			for (system = 0; system < Universe().length
					&& !(Universe()[system].SpecialEventType() == SpecialEventType.NA && Universe()[system]
							.TechLevel() == TechLevel.HiTech); system++)
				;
		...
		if (goodUniverse
				&& !FindDistantSystem(StarSystemId.Nix,
		...
		// doesn't already have a special event.
		if (goodUniverse
				&& !FindDistantSystem(StarSystemId.Gemulon,
		...
		// doesn't already have a special event.
		if (goodUniverse
				&& !FindDistantSystem(StarSystemId.Daled,
		...
		// doesn't already have a special event.
		if (goodUniverse
				&& !FindDistantSystem(StarSystemId.Endor,
		...
		if (goodUniverse) {
			for (int i = 0; i < Consts.SpecialEvents.length; i++) {
				for (int j = 0; j < Consts.SpecialEvents[i].Occurrence(); j++) {
					...
					Universe()[system].SpecialEventType(Consts.SpecialEvents[i]
							.Type());
				}
			}
		}
	...
	public void RecalculateBuyPrices(StarSystem system) {
		for (int i = 0; i < Consts.TradeItems.length; i++) {
			if (!system.ItemTraded(Consts.TradeItems[i]))
				...
				// (minimum is 1, max 12))
				_priceCargoBuy[i] = _priceCargoBuy[i]
						* (103 + Consts.MaxSkill - Commander().getShip()
								.Trader()) / 100;
	...
	public void SelectNextSystemWithinRange(boolean forward) {
		...
		if (dest.length > 0) {
			int index = spacetrader.util.Util.BruteSeek(dest, WarpSystemId()
					.CastToInt());

			if (index < 0)
				index = forward ? 0 : dest.length - 1;
			else
				index = (dest.length + index + (forward ? 1 : -1))
						% dest.length;

			if (Functions.WormholeExists(Commander().getCurrentSystem(),
					Universe()[dest[index]])) {
	...
	public void ShowNewspaper() {
		if (!getPaidForNewspaper()) {
			...
			if (Commander().getCash() < cost)
				GuiFacade.alert(AlertType.ArrivalIFNewspaper,
						Functions.Multiples(cost, "credit"));
			else if (Options().getNewsAutoPay()
					|| GuiFacade.alert(AlertType.ArrivalBuyNewspaper,
	...
	Hashtable Serialize() {
		...
		hash.add("_veryRareEncounters",
				ArrayListToIntArray(_veryRareEncounters));
	...
	public boolean Travel() {
		...
		if (getQuestStatusExperiment() == SpecialEvent.StatusExperimentPerformed
				&& getFabricRipProbability() > 0
				&& (getFabricRipProbability() == Consts.FabricRipInitialProbability || Functions
						.GetRandom(100) < getFabricRipProbability())) {
			GuiFacade.alert(AlertType.SpecialTimespaceFabricRip);
			SelectedSystemId(StarSystemId.FromInt(Functions
					.GetRandom(Universe().length)));
		}
		...
		while (getClicks() > 0) {
			...
			if (DetermineEncounter()) {
				...
				EncounterResult result = GuiFacade
						.performEncounter(getParentWindow());
	...
	public void Warp(boolean viaSingularity) {
		if (Commander().getDebt() > Consts.DebtTooLarge)
			...
			if (Commander().getShip().WildOnBoard()
					&& !Commander().getShip().HasWeapon(WeaponType.BeamLaser,
							false)) {
				if (GuiFacade.alert(AlertType.WildWontStayAboardLaser,
						Commander().getCurrentSystem().Name()) == DialogResult.Cancel)
					wildOk = false;
				else {
					GuiFacade.alert(AlertType.WildLeavesShip, Commander()
			...
			if (wildOk) {
				setArrivedViaWormhole(Functions.WormholeExists(Commander()
				...
				if (Travel()) {
					...
					// pod.
					/*
					 * if (Clicks == 0) FormAlert.Alert(AlertType.TravelArrival,
					 * ParentWindow);
					 */
				} else
	...
	public int CurrentCosts() {
		return InsuranceCosts() + InterestCosts() + MercenaryCosts()
				+ WormholeCosts();
	}
	...
	public String EncounterAction() {
		...
		if (getOpponentDisabled())
			action = Functions.StringVars(Strings.EncounterActionOppDisabled,
					EncounterShipText());
		else if (getEncounterOppFleeing()) {
			if (getEncounterType() == spacetrader.enums.EncounterType.PirateSurrender
					|| getEncounterType() == spacetrader.enums.EncounterType.TraderSurrender)
				action = Functions.StringVars(
						Strings.EncounterActionOppSurrender,
						EncounterShipText());
			else
				action = Functions.StringVars(
						Strings.EncounterActionOppFleeing, EncounterShipText());
		} else
			action = Functions.StringVars(Strings.EncounterActionOppAttacks,
					EncounterShipText());
	...
	public String EncounterText() {
		...
		if (getEncounterCmdrFleeing())
			cmdrStatus = Functions.StringVars(
					Strings.EncounterActionCmdrChased, EncounterShipText());
		else if (getEncounterOppHit())
			cmdrStatus = Functions.StringVars(Strings.EncounterActionOppHit,
					EncounterShipText());
		else
			cmdrStatus = Functions.StringVars(Strings.EncounterActionOppMissed,
					EncounterShipText());

		if (getEncounterOppFleeingPrev())
			oppStatus = Functions.StringVars(Strings.EncounterActionOppChased,
					EncounterShipText());
		else if (getEncounterCmdrHit())
			oppStatus = Functions.StringVars(Strings.EncounterActionCmdrHit,
					EncounterShipText());
		else
			oppStatus = Functions.StringVars(Strings.EncounterActionCmdrMissed,
					EncounterShipText());
	...
	public String EncounterTextInitial() {
		...
		return Functions.StringVars(Strings.EncounterText, new String[] {
				Functions.Multiples(getClicks(), Strings.DistanceSubunit),
				WarpSystem().Name(), encounterPretext,
				getOpponent().Name().toLowerCase() });
	}

	public int InsuranceCosts() {
		return Commander().getInsurance() ? (int) Math.max(1, Commander()
				.getShip().BaseWorth(true)
				* Consts.InsRate
				* (100 - Commander().NoClaim()) / 100) : 0;
	}

	public int InterestCosts() {
		return Commander().getDebt() > 0 ? (int) Math.max(1, Commander()
				.getDebt() * Consts.IntRate) : 0;
	}

	public int MercenaryCosts() {
		int total = 0;

		for (int i = 1; i < Commander().getShip().Crew().length
				&& Commander().getShip().Crew()[i] != null; i++)
	...
	public String NewspaperHead() {
		String[] heads = Strings.NewsMastheads[Commander().getCurrentSystem()
				.PoliticalSystemType().CastToInt()];
		String head = heads[Commander().getCurrentSystem().Id().CastToInt()
				% heads.length];

		return Functions
				.StringVars(head, Commander().getCurrentSystem().Name());
	}
	...
	public String NewspaperText() {
		...
		int minProbability = Consts.StoryProbability
				* curSys.TechLevel().CastToInt() + 10
			...
			if (Universe()[i].DestOk() && Universe()[i] != curSys) {
				...
				if (Universe()[i].SpecialEventType() != SpecialEventType.NA) {
					if (Universe()[i].SpecialEventType() == SpecialEventType.Moon)
						items.add(Functions.StringVars(Strings.NewsMoonForSale,
								Universe()[i].Name()));
					else if (Universe()[i].SpecialEventType() == SpecialEventType.TribbleBuyer)
						items.add(Functions.StringVars(
								Strings.NewsTribbleBuyer, Universe()[i].Name()));
				}
				if (Universe()[i].ShipyardId() != ShipyardId.NA)
					items.add(Functions.StringVars(Strings.NewsShipyard,
							Universe()[i].Name()));

				// And not-always-shown stories
				if (Universe()[i].SystemPressure() != SystemPressure.None
						&& Functions.GetRandom2(100) <= Consts.StoryProbability
								* curSys.TechLevel().CastToInt() + 10
								* (5 - Difficulty().CastToInt())) {
					int index = Functions
							.GetRandom2(Strings.NewsPressureExternal.length);
					String baseStr = Strings.NewsPressureExternal[index];
					String pressure = Strings.NewsPressureExternalPressures[Universe()[i]
							.SystemPressure().CastToInt()];
					items.add(Functions.StringVars(baseStr, pressure,
							Universe()[i].Name()));
					realNews = true;
				}
			}
		...
		if (!realNews) {
			String[] headlines = Strings.NewsHeadlines[curSys
					.PoliticalSystemType().CastToInt()];
		...
		return Util.StringsJoin(Strings.newline + Strings.newline,
	...
	public int Score() {
		...
		case Retired:
			...
			break;
		case BoughtMoon:
			daysMoon = Math.max(0, (Difficulty().CastToInt() + 1) * 100
					- Commander().getDays());
			modifier = 100;
			break;
		}

		return (Difficulty().CastToInt() + 1) * modifier
				* (daysMoon * 1000 + worth) / 250000;
	}
	...
	public StarSystem SelectedSystem() {
		return (_selectedSystemId == StarSystemId.NA ? null
				: Universe()[_selectedSystemId.CastToInt()]);
	}
	...
	public void TargetWormhole(boolean value) {
		...
		if (_targetWormhole) {
			int wormIndex = Util.BruteSeek(Wormholes(), SelectedSystemId()
					.CastToInt());
			_warpSystemId = StarSystemId.FromInt(Wormholes()[(wormIndex + 1)
					% Wormholes().length]);
		}
	}

	public StarSystem TrackedSystem() {
		return (_trackedSystemId == StarSystemId.NA ? null
				: Universe()[_trackedSystemId.CastToInt()]);
	}
	...
	public StarSystem WarpSystem() {
		return (_warpSystemId == StarSystemId.NA ? null
				: Universe()[_warpSystemId.CastToInt()]);
	...
	public int WormholeCosts() {
		return Functions.WormholeExists(Commander().getCurrentSystem(),
				WarpSystem()) ? Consts.WormDist
