...
public class ShipyardBox extends jwinforms.GroupBox {
	...
	public void Update() {
		if (commander == null) {
			...
			boolean noTech = (commander.getCurrentSystem().TechLevel()
					.CastToInt() < Consts.ShipSpecs[ShipType.Flea.CastToInt()]
					.MinimumTechLevel().CastToInt());

			lblShipsForSale.setText(noTech ? Strings.ShipyardShipNoSale
					: Strings.ShipyardShipForSale);
			btnBuyShip.setVisible(true);
			btnDesign
					.setVisible(commander.getCurrentSystem().Shipyard() != null);

			lblEquipForSale.setText(noTech ? Strings.ShipyardEquipNoSale
					: Strings.ShipyardEquipForSale);
