...
public class FormAlert extends SpaceTraderForm {
	...
	public FormAlert(String title, String text, String button1Text,
			DialogResult button1Result, String button2Text,
		...
		lblText.setWidth(g.MeasureString(
				(text.length() > 80 ? _80_CHARS : text), this.getFont()).width + 25);
		...
		btn1.setWidth(Math.max(40,
				g.MeasureString(btn1.getText(), this.getFont()).width + 35));
		btnWidth = btn1.getWidth();
		if (button2Text != null) {
			btn2.setText(button2Text);
			btn2.setWidth(Math.max(
					(int) Math.ceil(g.MeasureString(btn2.getText(),
							this.getFont()).width) + 10, 40));
	...
	private static FormAlert MakeDialog(AlertType type, String[] args) {
		switch (type) {
		case Alert:
			return (new FormAlert("^1", "^2", "Ok", DialogResult.OK, null,
		...
		case ArrivalBuyNewspaper:
			return (new FormAlert("Buy Newspaper?",
					"The local newspaper costs ^1. Do you wish to buy a copy?",
					"Buy Newspaper", DialogResult.Yes, "Cancel",
					DialogResult.No, args));
		case ArrivalIFFuel:
			return (new FormAlert("No Full Tanks",
					"You do not have enough money to buy full tanks.", "Ok",
		...
		case ArrivalIFNewspaper:
			return (new FormAlert(
					"Can't Afford it!",
					"Sorry! A newspaper costs ^1 in this system. You don't have enough money!",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		...
		case ArtifactRelinquished:
			return (new FormAlert("Artifact Relinquished",
					"The aliens take the artifact from you.", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case CargoIF:
			return (new FormAlert(
					"Not Enough Money",
		...
		case CargoNoEmptyBays:
			return (new FormAlert(
					"No Empty Bays",
					"You don't have any empty cargo holds available at the moment",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case CargoNoneAvailable:
			return (new FormAlert("Nothing Available",
					"None of these goods are available.", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case CargoNoneToSell:
			return (new FormAlert("None To ^1",
					"You have none of these goods in your cargo bays.", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case CargoNotInterested:
			return (new FormAlert(
					...
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case CargoNotSold:
			return (new FormAlert("Not Available",
					"That item is not available in this system.", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case ChartJump:
			return (new FormAlert(
					"Use Singularity?",
					"Do you wish to use the Portable Singularity to transport immediately to ^1?",
					"Use Singularity", DialogResult.Yes, "Don't use it",
					DialogResult.No, args));
		...
		case ChartJumpNoSystemSelected:
			return (new FormAlert(
					"No System Selected",
					"To use the Portable Singularity, track a system before clicking on this button.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case ChartTrackSystem:
			return (new FormAlert("Track System?", "^1?", "Yes",
					DialogResult.Yes, "No", DialogResult.No, args));
		case ChartWormholeUnreachable:
			return (new FormAlert("Wormhole Unreachable",
					"The wormhole to ^1 is only accessible from ^2.", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case Cheater:
			return (new FormAlert(
					"Cheater!",
					"Cheaters never prosper!  (Well, not with that command, anyway.)",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case CrewFireMercenary:
			return (new FormAlert("Fire Mercenary",
					"Are you sure you wish to fire ^1?", "Yes",
					DialogResult.Yes, "No", DialogResult.No, args));
		case CrewNoQuarters:
			return (new FormAlert("No Quarters Available",
					"You do not have any crew quarters available for ^1.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case DebtNoBuy:
			return (new FormAlert("You Have A Debt",
					"You can't buy that as long as you have debts.", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case DebtNone:
			return (new FormAlert("No Debt", "You have no debts.", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case DebtReminder:
			return (new FormAlert(
					"Loan Notification",
		...
		case DebtTooLargeLoan:
			return (new FormAlert("Debt Too High",
					"Your debt is too high to get another loan.", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case DebtTooLargeTrade:
			return (new FormAlert("Large Debt",
					"Your debt is too large.  Nobody will trade with you.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case DebtWarning:
			return (new FormAlert(
					"Warning: Large Debt",
		...
		case EncounterAttackCaptain:
			return (new FormAlert(
					...
					"Really Attack", DialogResult.Yes, "OK, I Won't",
					DialogResult.No, args));
		...
		case EncounterAttackNoLasers:
			return (new FormAlert(
					"No Hull-Damaging Weapons",
					"You only have disabling weapons, but your opponent cannot be disabled!",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case EncounterAttackNoWeapons:
			return (new FormAlert("No Weapons",
					"You can't attack without weapons!", "Ok", DialogResult.OK,
		...
		case EncounterDrinkContents:
			return (new FormAlert(
					...
					"Yes, Drink It", DialogResult.Yes, "No", DialogResult.No,
					args));
		case EncounterDumpAll:
			return (new FormAlert(
					"Dump All?",
					"You paid ^1 credits for these items. Are you sure you want to just dump them?",
					"Yes", DialogResult.Yes, "No", DialogResult.No, args));
		...
		case EncounterMarieCeleste:
			return (new FormAlert(
					...
					"Yes, Take Cargo", DialogResult.Yes, "No", DialogResult.No,
					args));
		...
		case EncounterOpponentEscaped:
			return (new FormAlert("Opponent Escaped",
					"Your opponent has managed to escape.", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case EncounterPiratesBounty:
			return (new FormAlert("Bounty",
					"You ^1 the pirate ship^2 and earned a bounty of ^3.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		...
		case EncounterPoliceBribe:
			return (new FormAlert(
					"Bribe",
					"These police officers are willing to forego inspection for the amount of ^1.",
					"Offer Bribe", DialogResult.Yes, "Forget It",
					DialogResult.No, args));
		case EncounterPoliceBribeCant:
			return (new FormAlert("No Bribe",
					"These police officers can't be bribed.", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case EncounterPoliceBribeLowCash:
			return (new FormAlert("Not Enough Cash",
		...
		case EncounterPoliceNothingIllegal:
			return (new FormAlert(
					...
					"Yes, I still want to", DialogResult.Yes, "OK, I won't",
					DialogResult.No, args));
		case EncounterPoliceSubmit:
			return (new FormAlert(
					"You Have Illegal Goods",
					"Are you sure you want to let the police search you? You are carrying ^1! ^2",
					"Yes, let them", DialogResult.Yes, "No", DialogResult.No,
					args));
		...
		case EncounterScoop:
			return (new FormAlert(
					...
					"Pick It Up", DialogResult.Yes, "Let It Go",
					DialogResult.No, args));
		case EncounterScoopNoRoom:
			return (new FormAlert(
					...
					"Make Room", DialogResult.Yes, "Let it go",
					DialogResult.No, args));
		case EncounterScoopNoScoop:
			return (new FormAlert(
					"No Scoop",
					"You regret finding nothing in your holds that can be dumped, and let the canister go.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case EncounterSurrenderRefused:
			return (new FormAlert("To The Death!",
					"Surrender? Hah! We want your HEAD!", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case EncounterTonicConsumedGood:
			return (new FormAlert(
					...
					"Mmmmm. Captain Marmoset's Amazing Skill Tonic not only fills you with energy, but tastes like a fine single-malt."
							+ Strings.newline, "Ok", DialogResult.OK, null,
					DialogResult.None, args));
		...
		case EncounterTradeCompleted:
			return (new FormAlert(
					"Trade Completed",
					"Thanks for ^1 the ^2. It's been a pleasure doing business with you.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case EncounterYouLose:
			return (new FormAlert("You Lose",
					"Your ship has been destroyed by your opponent.", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case EncounterYouWin:
			return (new FormAlert("You Win",
					"You have destroyed your opponent.", "Ok", DialogResult.OK,
					null, DialogResult.None, args));
		case EquipmentAlreadyOwn:
			return (new FormAlert("You Already Have One",
					"It's not useful to buy more than one of this item.", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case EquipmentBuy:
			return (new FormAlert("Buy ^1",
					"Do you wish to buy this item for ^2 credits?", "Yes",
					DialogResult.Yes, "No", DialogResult.No, args));
		case EquipmentEscapePod:
			return (new FormAlert("Escape Pod",
		...
		case EquipmentFuelCompactor:
			return (new FormAlert("Fuel Compactor",
					"You now have a fuel compactor installed on your ship.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		...
		case EquipmentIF:
			return (new FormAlert("Not Enough Money",
					"You don't have enough money to spend on this item.", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case EquipmentLightningShield:
			return (new FormAlert(
					"Lightning Shield",
					"You now have one lightning shield installed on your ship.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		...
		case EquipmentQuantumDisruptor:
			return (new FormAlert(
					"Quantum Disruptor",
					"You now have one quantum disruptor installed on your ship.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case EquipmentSell:
			return (new FormAlert("Sell Item",
					"Are you sure you want to sell this item?", "Yes",
					DialogResult.Yes, "No", DialogResult.No, args));
		case FileErrorOpen:
			return (new FormAlert("Error",
					"An error occurred while trying to open ^1."
							+ Strings.newline + Strings.newline + "^2", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case FileErrorSave:
			return (new FormAlert("Error",
					"An error occurred while trying to save ^1."
							+ Strings.newline + Strings.newline + "^2", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case FleaBuilt:
			return (new FormAlert(
		...
		case GameAbandonConfirm:
			return (new FormAlert("Are You Sure?",
					"Are you sure you want to abandon your current game?",
					"Yes", DialogResult.Yes, "No", DialogResult.No, args));
		case GameClearHighScores:
			return (new FormAlert("Clear High Scores",
					"Are you sure you wish to clear the high score table?",
					"Yes", DialogResult.Yes, "No", DialogResult.No, args));
		case GameEndBoughtMoon:
			return (new FormAlert("You Have Retired",
					GameEndType.BoughtMoon.CastToInt()));
		case GameEndBoughtMoonGirl:
			return (new FormAlert("You Have Retired with the Princess",
					GameEndType.BoughtMoonGirl.CastToInt()));
		case GameEndHighScoreAchieved:
			return (new FormAlert("Congratulations!",
					"You have made the high-score list!", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case GameEndHighScoreCheat:
			return (new FormAlert(
					"Naughy, Naughty!",
					"You would have made the high-score list if you weren't a Cheat!.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case GameEndHighScoreMissed:
			return (new FormAlert("Sorry",
					"Alas! This is not enough to enter the high-score list.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case GameEndKilled:
			return (new FormAlert("You Are Dead",
					GameEndType.Killed.CastToInt()));
		case GameEndRetired:
			return (new FormAlert("You Have Retired",
					GameEndType.Retired.CastToInt()));
		case GameEndScore:
			return (new FormAlert("Score", "You achieved a score of ^1.^2%.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case GameRetire:
			return (new FormAlert("Retire", "Are you sure you wish to retire?",
					"Yes", DialogResult.Yes, "No", DialogResult.No, args));
		case InsuranceNoEscapePod:
			return (new FormAlert(
					"No Escape Pod",
					"Insurance isn't useful for you, since you don't have an escape pod.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		...
		case InsuranceStop:
			return (new FormAlert(
					...
					"Yes", DialogResult.Yes, "No", DialogResult.No, args));
		case JailConvicted:
			return (new FormAlert("Convicted",
					"You are convicted to ^1 in prison and a fine of ^2.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		...
		case JailMercenariesLeave:
			return (new FormAlert("Mercenaries Leave",
		...
		case LeavingIFWormholeTax:
			return (new FormAlert("Wormhole Tax",
		...
		case MeetCaptainAhab:
			return (new FormAlert(
					...
					"Yes, Trade Shield", DialogResult.Yes, "No",
					DialogResult.No, args));
		case MeetCaptainConrad:
			return (new FormAlert(
					...
					"Yes, Trade Laser", DialogResult.Yes, "No",
					DialogResult.No, args));
		case MeetCaptainHuie:
			return (new FormAlert(
					...
					DialogResult.No, args));
		case NewGameConfirm:
			return (new FormAlert("New Game",
					"Are you sure you wish to start a new game?", "Yes",
					DialogResult.Yes, "No", DialogResult.No, args));
		case NewGameMoreSkillPoints:
			return (new FormAlert("More Skill Points",
					"You haven't awarded all 20 skill points yet.", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case OptionsNoGame:
			return (new FormAlert(
					"No Game Active",
					"You don't have a game open, so you can only change the default options.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case PreciousHidden:
			return (new FormAlert(
		...
		case RegistryError:
			return (new FormAlert("Error...",
					"Error accessing the Registry: ^1", "Ok", DialogResult.OK,
					null, DialogResult.None, args));
		case SculptureConfiscated:
			return (new FormAlert(
					"Police Confiscate Sculpture",
					"The Police confiscate the stolen sculpture and return it to its rightful owner.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case SculptureSaved:
			return (new FormAlert(
					"Sculpture Saved",
		...
		case ShipBuyConfirm:
			return (new FormAlert(
					"Buy New Ship",
					"Are you sure you wish to trade in your ^1 for a new ^2^3?",
					"Yes", DialogResult.Yes, "No", DialogResult.No, args));
		...
		case ShipBuyIF:
			return (new FormAlert("Not Enough Money",
					"You don't have enough money to buy this ship.", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		...
		case ShipBuyPassengerQuarters:
			return (new FormAlert(
					...
					"You must get a ship with enough crew quarters so that ^1 can stay on board.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case ShipBuyReactor:
			return (new FormAlert(
					"Shipyard Engineer",
		...
		case ShipBuyTransfer:
			return (new FormAlert("Transfer ^1",
					"I'll transfer your ^2 to your new ship for ^3 credits.",
					"Do it!", DialogResult.Yes, "No thanks", DialogResult.No,
					args));
		case ShipDesignIF:
			return (new FormAlert("Not Enough Money",
					"You don't have enough money to create this design.", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case ShipDesignThanks:
			return (new FormAlert("Thank you!",
					"^1 thanks you for your business!", "Ok", DialogResult.OK,
					null, DialogResult.None, args));
		...
		case SpecialCleanRecord:
			return (new FormAlert("Clean Record",
		...
		case SpecialIF:
			return (new FormAlert(
					"Not Enough Money",
					"You don't have enough cash to spend to accept this offer.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		...
		case SpecialNoQuarters:
			return (new FormAlert("No Free Quarters",
					"There are currently no free crew quarters on your ship.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case SpecialNotEnoughBays:
			return (new FormAlert("Not Enough Bays",
					"You don't have enough empty cargo bays at the moment.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case SpecialPassengerConcernedJarek:
			return (new FormAlert(
					"Ship's Comm.",
					"Commander? Jarek here. Do you require any assitance in charting a course to Devidia?",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case SpecialPassengerConcernedPrincess:
			return (new FormAlert(
					"Ship's Comm.",
					"[Ziyal] Oh Captain? (giggles) Would it help if I got out and pushed?",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case SpecialPassengerConcernedWild:
			return (new FormAlert(
					"Ship's Comm.",
					"Bridge? This is Jonathan. Are we there yet? Heh, heh. Sorry, I couldn't resist.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case SpecialPassengerImpatientJarek:
			return (new FormAlert(
					"Ship's Comm.",
					"Captain! This is the Ambassador speaking. We should have been there by now?!",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case SpecialPassengerImpatientPrincess:
			return (new FormAlert(
					"Ship's Comm.",
					"Sir! Are you taking me home or merely taking the place of my previous captors?!",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case SpecialPassengerImpatientWild:
			return (new FormAlert("Ship's Comm.",
					"Commander! Wild here. What's taking us so long?!", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		...
		case SpecialSkillIncrease:
			return (new FormAlert("Skill Increase",
					"The alien increases one of your skills. ", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case SpecialTimespaceFabricRip:
			return (new FormAlert(
					...
					"You have flown through a tear in the timespace continuum caused by Dr. Fehler's failed experiment. You may not have reached"
							+ Strings.newline + " your planned destination!",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case SpecialTrainingCompleted:
			return (new FormAlert(
					...
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case TravelArrival:
			return (new FormAlert("Arrival", "You arrive at your destination.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		...
		case TribblesHalfDied:
			return (new FormAlert(
					...
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case TribblesKilled:
			return (new FormAlert("Tribbles Killed",
					"Your tribbles all died in the explosion.", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case TribblesMostDied:
			return (new FormAlert(
					"Most Tribbles Died",
					"You find that, instead of narcotics, some of your cargo bays contain only dead tribbles!",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case TribblesOwn:
			return (new FormAlert(
					"A Tribble",
		...
		case TribblesRemoved:
			return (new FormAlert("Tribbles Removed",
					"The tribbles were sold with your ship.", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		case TribblesInspector:
			return (new FormAlert(
					...
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case TribblesSqueek:
			return (new FormAlert("A Tribble", "Squeek!", "Ok",
					DialogResult.OK, null, DialogResult.None, args));
		...
		case WildLeavesShip:
			return (new FormAlert(
					"Wild Leaves Ship",
					"Jonathan Wild leaves your ship, and goes into hiding on ^1.",
					"Ok", DialogResult.OK, null, DialogResult.None, args));
		case WildSculpture:
			return (new FormAlert(
					"Wild Eyes Sculpture",
		...
		case WildWontStayAboardLaser:
			return (new FormAlert(
					...
					"Jonathan Wild isn't about to go with you if you're not armed with at least a Beam Laser. He'd rather take his chances hiding out here on ^1."
							+ Strings.newline, "Say Goodbye to Wild",
					DialogResult.OK, "Cancel", DialogResult.Cancel, args));
