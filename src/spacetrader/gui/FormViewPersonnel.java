...
public class FormViewPersonnel extends SpaceTraderForm {
	...
	private void UpdateCurrentCrew() {
		...
		if (entries)
			lstCrew.setHeight(lstCrew.getItemHeight()
					* Math.min(lstCrew.Items.size(), 6) + 2);
		else
	...
	private void UpdateForHire() {
		CrewMember[] mercs = game.Commander().getCurrentSystem()
				.MercenariesForHire();
		...
		if (entries)
			lstForHire.setHeight(lstForHire.getItemHeight()
					* Math.min(lstForHire.Items.size(), 6) + 2);
		else
			// TODO: remove this when Strings are moved to resource.
	...
	private void UpdateInfo() {
		...
		if (selectedCrewMember != null) {
			...
			lblRate.setText(Functions.StringVars(Strings.MoneyRateSuffix,
			...
			btnHireFire.setText(game.Commander().getShip()
					.HasCrew(selectedCrewMember.Id()) ? Strings.MercenaryFire
					: Strings.MercenaryHire);
			hireFireVisible = rateVisible
					|| selectedCrewMember.Id() == CrewMemberId.Zeethibal;
		}
	...
	private void HireFire(Object sender, EventArgs e) {
		if (selectedCrewMember != null && btnHireFire.getVisible()) {
			if (game.Commander().getShip().HasCrew(selectedCrewMember.Id())) {
				if (GuiFacade.alert(AlertType.CrewFireMercenary,
						selectedCrewMember.Name()) == DialogResult.Yes) {
				...
				if (game.Commander().getShip().FreeCrewQuarters() == 0)
					GuiFacade.alert(AlertType.CrewNoQuarters,
