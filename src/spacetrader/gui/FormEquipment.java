...
public class FormEquipment extends SpaceTraderForm {
	...
	private void InitializeComponent() {
		...
		this.boxSell.setSize(new Dimension(144, 304));
		...
		this.lblSellGadgetNoSlots.setSize(new Dimension(104, 16));
		...
		this.lblSellShieldNoSlots.setSize(new Dimension(104, 16));
		...
		this.lblSellWeaponNoSlots.setSize(new Dimension(104, 16));
		...
		this.lblSellGadgets.setSize(new Dimension(47, 16));
		...
		this.lblSellShields.setSize(new Dimension(41, 16));
		...
		this.lblSellWeapons.setSize(new Dimension(52, 16));
		...
		this.lstSellGadget.setSize(new Dimension(128, 67));
		this.lstSellGadget.setTabIndex(3);
		this.lstSellGadget
				.setDoubleClick(new EventHandler<Object, EventArgs>() {
					@Override
		...
		this.lstSellGadget
				.setSelectedIndexChanged(new EventHandler<Object, EventArgs>() {
					@Override
					public void handle(Object sender, jwinforms.EventArgs e) {
		...
		this.lstSellShield.setSize(new Dimension(128, 67));
		...
		this.lstSellWeapon.setSize(new Dimension(128, 67));
		...
		this.boxBuy.setSize(new Dimension(144, 304));
		...
		this.lblBuyGadgetNone.setSize(new Dimension(104, 16));
		...
		this.lblBuyShieldNone.setSize(new Dimension(104, 16));
		...
		this.lblBuyWeaponNone.setSize(new Dimension(104, 16));
		...
		this.lblBuyGadgets.setSize(new Dimension(47, 16));
		...
		this.lblBuyShields.setSize(new Dimension(41, 16));
		...
		this.lblBuyWeapons.setSize(new Dimension(52, 16));
		...
		this.lstBuyGadget.setSize(new Dimension(128, 67));
		...
		this.lstBuyShield.setSize(new Dimension(128, 67));
		...
		this.lstBuyWeapon.setSize(new Dimension(128, 67));
		...
		this.boxShipInfo.setSize(new Dimension(208, 304));
		...
		this.lblCharge.setSize(new Dimension(116, 16));
		...
		this.lblPower.setSize(new Dimension(116, 16));
		...
		this.lblChargeLabel.setSize(new Dimension(46, 16));
		...
		this.lblPowerLabel.setSize(new Dimension(41, 16));
		...
		this.lblType.setSize(new Dimension(116, 16));
		...
		this.lblTypeLabel.setSize(new Dimension(34, 16));
		...
		this.lblNameLabel.setSize(new Dimension(39, 16));
		...
		this.btnSell.setSize(new Dimension(58, 22));
		...
		this.btnBuy.setSize(new Dimension(58, 22));
		...
		this.lblBuyPriceLabel.setSize(new Dimension(58, 16));
		...
		this.lblBuyPrice.setSize(new Dimension(116, 16));
		...
		this.lblSellPriceLabel.setSize(new Dimension(58, 16));
		...
		this.picEquipment.setSize(new Dimension(66, 54));
		...
		this.lblSellPrice.setSize(new Dimension(116, 16));
		...
		this.lblDescription.setSize(new Dimension(196, 75));
		...
		this.lblName.setSize(new Dimension(116, 16));
		...
		this.setClientSize(new Dimension(522, 311));
	...
	private void Buy() {
		if (selectedEquipment != null && !sellSideSelected) {
			...
			if (baseType == EquipmentType.Gadget
					&& cmdr.getShip().HasGadget(
							((Gadget) selectedEquipment).Type())
			...
			else if ((baseType == EquipmentType.Weapon && cmdr.getShip()
					.FreeSlotsWeapon() == 0)
					|| (baseType == EquipmentType.Shield && cmdr.getShip()
							.FreeSlotsShield() == 0)
					|| (baseType == EquipmentType.Gadget && cmdr.getShip()
							.FreeSlotsGadget() == 0))
				GuiFacade.alert(AlertType.EquipmentNotEnoughSlots);
			else if (GuiFacade.alert(AlertType.EquipmentBuy,
	...
	private void Sell() {
		if (selectedEquipment != null && sellSideSelected) {
			if (GuiFacade.alert(AlertType.EquipmentSell) == DialogResult.Yes) {
				...
				int slot = lstSellWeapon.getSelectedIndex()
						+ lstSellShield.getSelectedIndex()
	...
	private void UpdateBuy() {
		...
		ListBox[] buyBoxes = new ListBox[] { lstBuyWeapon, lstBuyShield,
				lstBuyGadget };
		Label[] buyLabels = new Label[] { lblBuyWeaponNone, lblBuyShieldNone,
				lblBuyGadgetNone };
		for (int i = 0; i < buyBoxes.length; i++) {
			...
			if (entries)
				buyBoxes[i].setHeight(buyBoxes[i].getItemHeight()
						* Math.min(buyBoxes[i].Items.size(), 5) + 2);
		}
	}
	...
	private void UpdateInfo() {
		...
		if (selectedEquipment == null) {
			...
			case Shield:
				...
				charge = sellSideSelected ? ""
						+ ((Shield) selectedEquipment).getCharge() : Strings.NA;
				break;
			case Gadget:
				power = Strings.NA;
				charge = Strings.NA;
			...
			lblType.setText(Strings.EquipmentTypes[selectedEquipment
					.EquipmentType().CastToInt()]);
			lblDescription
					.setText(Strings.EquipmentDescriptions[selectedEquipment
							.EquipmentType().CastToInt()][selectedEquipment
							.SubType().CastToInt()]);
			lblBuyPrice
					.setText(Functions.FormatMoney(selectedEquipment.Price()));
			lblSellPrice.setText(Functions.FormatMoney(selectedEquipment
					.SellPrice()));
			lblPower.setText(power);
			lblCharge.setText(charge);
			picEquipment.setImage(selectedEquipment.Image());
			btnBuy.setVisible(!sellSideSelected
	...
	private void UpdateSell() {
		...
		for (index = 0; index < equipSell.length; index++)
			lstSellWeapon.Items
					.add(equipSell[index] == null ? (Object) Strings.EquipmentFreeSlot
							: equipSell[index]);
		...
		ListBox[] sellBoxes = new ListBox[] { lstSellWeapon, lstSellShield,
				lstSellGadget };
		Label[] sellLabels = new Label[] { lblSellWeaponNoSlots,
				lblSellShieldNoSlots, lblSellGadgetNoSlots };
		for (int i = 0; i < sellBoxes.length; i++) {
			...
			if (entries)
				sellBoxes[i].setHeight(sellBoxes[i].getItemHeight()
						* Math.min(sellBoxes[i].Items.size(), 5) + 2);
		}
	}
