...
public class FormViewBank extends SpaceTraderForm {
	...
	private final int MaxLoan = Game.CurrentGame().Commander()
			.getPoliceRecordScore() >= Consts.PoliceRecordScoreClean ? Math
			.min(25000, Math.max(1000,
					Game.CurrentGame().Commander().Worth() / 5000 * 500)) : 500;
	...
	private void InitializeComponent() {
		...
		Controls.addAll(Arrays.asList(lblMaxNoClaim, btnClose, btnPayBack,
				...
				btnBuyInsurance, lblNoClaim, lblShipValue, btnGetLoan,
				lblMaxLoan, lblCurrentDebt));
	...
	private void UpdateAll() {
		...
		// Insurance Info
		lblShipValue.setText(Functions.FormatMoney(cmdr.getShip().BaseWorth(
				true)));
		...
		btnBuyInsurance.setText(Functions.StringVars(
				Strings.BankInsuranceButtonText,
				cmdr.getInsurance() ? Strings.BankInsuranceButtonStop
						: Strings.BankInsuranceButtonBuy));
	}
