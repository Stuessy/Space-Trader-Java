...
public class HighScoreRecord extends STSerializableObject implements
		Comparable<HighScoreRecord>// TODO implements Comparable
{
	...
	public HighScoreRecord(String name, int score, GameEndType type, int days,
			int worth, Difficulty difficulty) {
		_name = name;
		_score = score;
		_type = type;
		_days = days;
	...
	public HighScoreRecord(Hashtable hash) {
		...
		_type = GetValueFromHashEnum(hash, "_type", GameEndType.class);
		_days = GetValueFromHash(hash, "_days", Integer.class);
		_worth = GetValueFromHash(hash, "_worth", Integer.class);
		_difficulty = GetValueFromHashEnum(hash, "_difficulty",
				Difficulty.class);
	}
