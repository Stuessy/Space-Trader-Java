...
/**
 * This is kind-a temp class, to hold functions that are moved from the gui
 * classes (VIEW) downwards. There is already a CONTROLLER class (Called Game)
 * but it's just too big to factor everything directly into it.
 * 
 * I'm guessing that later on, all methods from this class will be distributed
 * around the various data classes, or into the Game class.
 * 
 * @author Aviv
 */
public class GameController {
	...
	public void ClearHighScores() {
		...
		Functions.SaveFile(Consts.HighScoreFile,
				STSerializableObject.ArrayToArrayList(highScores));
	}
	...
	public void GameEnd() {
		...
		GuiFacade.alert(AlertType.GameEndScore,
		...
		HighScoreRecord candidate = new HighScoreRecord(
				...
				game.Commander().getDays(), game.Commander().Worth(),
				game.Difficulty());
