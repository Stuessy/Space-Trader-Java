...
public class PoliticalSystem {
	...
	private final int _reactionIllegal; // Reaction level of illegal goods 0 =
										// total
	// acceptance (determines how police reacts
	// if they find you carry them)
	private final Activity _activityPolice; // Activity level of police force 0
											// = no
	...
	private final int _bribeLevel; // Indicates how easily someone can be bribed
									// 0 =
	// unbribeable/high bribe costs
	private final boolean _drugsOk; // Drugs can be traded (if not, people
									// aren't
	...
	// too strict)
	private final TradeItemType _wanted; // Tradeitem requested in particular in
											// this
	...
	public PoliticalSystem(PoliticalSystemType type, int reactionIllegal,
			Activity activityPolice, Activity activityPirates,
			Activity activityTraders, TechLevel minTechLevel,
			TechLevel maxTechLevel, int bribeLevel, boolean drugsOk,
			boolean firearmsOk, TradeItemType wanted) {
		_type = type;
	...
	public boolean ShipTypeLikely(ShipType shipType, OpponentType oppType) {
		...
		int diffMod = Math.max(0, Game.CurrentGame().Difficulty().CastToInt()
				- Difficulty.Normal.CastToInt());

		switch (oppType) {
		case Pirate:
			likely = ActivityPirates().CastToInt() + diffMod >= Consts.ShipSpecs[shipType
					.CastToInt()].Pirates().CastToInt();
			break;
		case Police:
			likely = ActivityPolice().CastToInt() + diffMod >= Consts.ShipSpecs[shipType
					.CastToInt()].Police().CastToInt();
			break;
		case Trader:
			likely = ActivityTraders().CastToInt() + diffMod >= Consts.ShipSpecs[shipType
					.CastToInt()].Traders().CastToInt();
			break;
		}
