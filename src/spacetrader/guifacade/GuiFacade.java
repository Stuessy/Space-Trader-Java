...
/**
 * Ment to be the connecting link between the game engien and the GUI. Will help
 * later with replacing the GUI layer.
 * 
 * It holds things refactored UP from the controller classes. Also see
 * GameController.
 * 
 * @author Aviv
 * 
...
public class GuiFacade {
	 ...
	 * @return amount selected by the user to buy; 0 if clicked Cancel.
	 */
	public static int queryAmountRelease(int item, CargoSellOp op,
			int maxAmount, int price) {
		FormCargoSell form = new FormCargoSell(item, maxAmount, op, price);
		return form.Show() == DialogResult.OK ? form.Amount() : 0;
	}

	/**
	 * Lets the user Plunder an un-opposing ship. The gui handles the what and
	 * the how much of it.
	 * 
	...
	/**
	 * Lets the user get rid of items. The gui handles the what and the how much
	 * of it.
	 * 
	 * todo smells like there's too much game code in that form.
	 */
